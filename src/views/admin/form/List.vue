<template>
  <v-container>
          ...
          <v-tooltip color="green" bottom>
            <template v-slot:activator="{ on, attrs }">
              <v-btn
                ...
                v-on="on"
                color="green"
                @click="$router.push({ path: '/new-form' })"
              >
    ...
    ></delete>
    <preview :_dialog="preview" :_form="form" v-on:dialogClose="(value) => {preview = value}"></preview>
  </v-container>
</template>
...
export default {
  ...
  methods: {
    editItem(item) {
      this.$router.push({
        path: `/admin/form/edit/${item._id}`,
      });
    },
    detailItem(item) {
      this.$router.push({
        path: `/admin/form/detail/${item._id}`,
      });
    },
    handleDelete() {
      ApiService.delete(`forms/id/${this.deleteRes.id}`).then((x) => {
        if (x.status == 200) {
